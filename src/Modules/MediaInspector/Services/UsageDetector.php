...
class UsageDetector {
	...
	public function auditMedia( int $limit = 50, int $offset = 0, string $filter = 'all' ): array {
		...
		foreach ( $attachments as $attachment ) {
			...
			if ( $is_huge_png ) {
				++$summary['huge_png_count'];
			}
			if ( $is_huge_jpg ) {
				++$summary['huge_jpg_count'];
			}
			...
			if ( $is_wrong_dimensions ) {
				++$summary['wrong_dimensions_count'];
			}
			...
			// Filtering
			if ( $filter === 'unused' && ! $is_unused ) {
				continue;
			}
			if ( $filter === 'missing_alt' && ! $has_missing_alt ) {
				continue;
			}
			if ( $filter === 'huge_png' && ! $is_huge_png ) {
				continue;
			}
			if ( $filter === 'huge_jpg' && ! $is_huge_jpg ) {
				continue;
			}
			if ( $filter === 'svg' && ! $is_svg ) {
				continue;
			}
			if ( $filter === 'duplicates' && ! $is_dup_filename && ! $is_dup_hash ) {
				continue;
			}
